	...
	<div id="body">
		...
		<div class="content">
			...
			<header class="case_right_f">
                <div class="case_right_f_l">Share Cooperation</div>
			...
			<h2>Cooperative Organization</h2>
			<c:if test="${!empty co}">
				<c:forEach items="${co}" var="co">
					<p style="margin-bottom: 0px;">	
						<a href="${co.COurl }">&nbsp;&nbsp;&nbsp;&nbsp;${co.COname }</a>
						<span>${co.COintroduce }</span>
					</p>
				</c:forEach>
			</c:if>
			<h2>Cooperative Scholar</h2>
			<c:if test="${!empty cs}">
				<c:forEach items="${cs}" var="cs">
					<p style="margin-bottom: 0px;">	
						<a href="${cs.CSurl }">&nbsp;&nbsp;&nbsp;&nbsp;${cs.CSname }</a>
						<span>${cs.CSintroduce }</span>
					</p>
				</c:forEach>
			</c:if>
	</div>
